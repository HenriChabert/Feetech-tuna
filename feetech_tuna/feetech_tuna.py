...
servoRegs = [
    ...
    },
    {
        ...
        "addr": SMS_STS_MOVING_THRESHOLD,
        "size": 1,
        "type": "uint8",
    },
    {"name": "DTs(ms)", "addr": SMS_STS_DTS, "size": 1, "type": "uint8"},
    {"name": "Vk(ms)", "addr": SMS_STS_VK, "size": 1, "type": "uint8"},
    {"name": "Vmin", "addr": SMS_STS_VMIN, "size": 1, "type": "uint8"},
    {"name": "Vmax", "addr": SMS_STS_VMAX, "size": 1, "type": "uint8"},
    {"name": "Amax", "addr": SMS_STS_AMAX, "size": 1, "type": "uint8"},
    {"name": "KAcc", "addr": SMS_STS_KACC, "size": 1, "type": "uint8"},
]
...
class FeetechTuna:
    ...
    def listRegs(self, servoId):
        ...
        for reg in servoRegs:
            try:
                value, comm_result, error = self.packetHandler.readTxRx(
                    servoId, reg["addr"], reg["size"]
                )
                if comm_result == COMM_SUCCESS:
                    if reg["size"] == 2:
                        value = self.packetHandler.scs_tohost(
                            self.packetHandler.scs_makeword(value[0], value[1]), 15
                        )
                    else:
                        value = value[0]
                    result.append(
                        {"name": reg["name"], "addr": reg["addr"], "value": value}
                    )
                else:
                    print("Failed to read register " + reg["name"])
                    print(
                        "Comm result: " + self.packetHandler.getTxRxResult(comm_result)
                    )
            except:
                print(
                    "Warning: Error occurred when reading register "
                    + reg["name"]
                    + " (addr: "
                    + str(reg["addr"])
                    + ")"
    ...
    def writeReg(self, servoId, regAddr, value):
        ...
        if reg["size"] == 2:
            value = [
                self.packetHandler.scs_lobyte(value),
                self.packetHandler.scs_hibyte(value),
            ]
        else:
            value = [value]
        ...
        while retries > 0:
            comm_result, error = self.packetHandler.writeTxRx(
                servoId, regAddr, reg["size"], value
            )
            if comm_result == COMM_SUCCESS:
